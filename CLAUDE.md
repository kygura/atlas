# Atlas — Autonomous Travel Organizer
...
Read `config/trip_preferences.json` for activity style, accommodation
preferences, budget envelope, and planning depth. If the file does not exist,
initialize it with the defaults defined in §Config Defaults below.
...
| Gate                  | Threshold (default)                  | Override               |
|-----------------------|--------------------------------------|------------------------|
| `composite_score`     | ≥ 3.0 / 5.0                         | `min_composite_score`  |
| `weather_score`       | ≥ 3 / 5                             | `min_weather_score`    |
| `crowd_level`         | not `peak`                           | `allow_peak_crowds`    |
| `distortion_flag`     | `false`                              | —                      |
| `budget_compliance`   | flight ≤ `budget_economy_eur`        | —                      |
| `isolation_score`     | ≥ 2 / 5 (if preference tag present)  | `min_isolation_score`  |
...
**Activity density**: 1–2 activities per day. Morning and afternoon slots.
Evenings are free unless the user explicitly requests evening activities.
